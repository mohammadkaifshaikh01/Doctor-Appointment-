...
const Navbar = () => {
  ...
  const { user, token, isAuth, setIsAuth, setUser, setToken } = useContext(AppContext);

  const readbleUser = JSON.parse(user)
  ...
  return (
    <div className="flex items-center justify-between text-sm py-3 mb-5 border-b border-b-gray-400">
      ...
      <ul className="hidden md:flex items-start gap-10 font-medium">
        ...
        <NavLink to="/about"> <li className="py-3">ABOUT</li> </NavLink>
        <NavLink to="/contact"> <li className="py-3">CONTACT</li> </NavLink>
      </ul>

      <div className="flex items-center gap-4">
        {isAuth ? (
          <div className="flex items-center gap-2 cursor-pointer group relative">
            <p>{readbleUser?.username || "User"}</p>
